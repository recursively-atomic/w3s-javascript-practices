...
const resetutton = document.getElementById('reset');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    resetutton.addEventListener('click', resetCount);
...
function saveCount() {
    const currentValue = parseInt(countDisplay.innerText);
    localStorage.setItem('count-event', currentValue);

    updateMessage(saveButton, `Count saved to ${currentValue}!`);
}

function loadCount() {
    const previousValue = localStorage.getItem('count-event');
...
function clearCount() {
    localStorage.removeItem('count-event');
